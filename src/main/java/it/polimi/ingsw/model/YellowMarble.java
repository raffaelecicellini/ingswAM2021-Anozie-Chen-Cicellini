...
public class YellowMarble implements Marble{
    ...
    /**
     * Constructor of the YellowMarble, called by getInstance()
     */
    private YellowMarble() {}
    /**
     * If the instance is not already created, it creates one.
     * @return: the instance of the YellowMarble
     */
    ...
    /**
     * This method defines the action performed by a YellowMarble. It tries to put a Coin in the chosen deposit. If this can't be done,
            ...
            if (current.getAmount()==0 || (current.getColor()==Color.YELLOW && current.getAmount()<(dep+1))){
                ...
                if (duplicate>=0 && deposits.get(duplicate).getAmount()<(duplicate+1)){
                    ...
                    if (checkSwap(deposits, duplicate)){
                        throw new InvalidActionException("You can't put the " + YellowMarble.getInstance() + " in the " + chosen + " deposit, but you can try doing a swap!");
                    }
