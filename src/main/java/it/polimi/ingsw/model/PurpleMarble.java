...
public class PurpleMarble implements Marble{
    ...
    private int checkDuplicates(ArrayList<ResourceAmount> deposits, int curr){
        for (int i=0; i<3; i++) {
            ...
            if (res.getColor()==Color.PURPLE && res.getAmount()>0 && i!=curr) {
                return i;
            }
        }
        return -1;
    ...
    private boolean checkSpace(ArrayList<ResourceAmount> deposits){
        ...
        for (int i=0; i<3; i++) {
            ResourceAmount res= deposits.get(i);
            if ((res.getColor()==Color.PURPLE && res.getAmount()>0 && res.getAmount()<(i+1)) || (res.getAmount()==0)) {
                ret=true;
            }
            else if(res.getColor()==Color.PURPLE && res.getAmount()==(i+1)){
