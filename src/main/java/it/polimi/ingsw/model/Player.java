...
public class Player {
    ...
    private void baseProduction(Map<String, String> info, ResourceAmount[] inputStrongbox, ArrayList<ResourceAmount> deps, ResourceAmount[] outputStrongbox)
        ...
        for (int i = 0; i < 2; i++) {
            ...
            if (index == 5) {
                ...
                if (!found)
                    throw new InvalidActionException("The resource is not in the specified position (strongbox)");
    ...
    public void chooseInitialResource(Message info) throws InvalidActionException {
        ...
        int pos1 = -1;
        int pos2 = -1;
        if (info.size() != numberInitialResource * 2)
            throw new InvalidActionException("You didn't select the right amount of resources.");
    ...
    public void swapDeposit(Message info) throws InvalidActionException {
        if (info.size() != 2)
            throw new InvalidActionException("Invalid action! Make sure to select the source and the destination!");
        String source = info.getSource();
        String dest = info.getDest();
        if (source == null || dest == null)
            throw new InvalidActionException("Invalid action! You didn't select the source and/or the destination!");
        if (parseChoice(source) >= 5 || parseChoice(dest) >= 5)
            throw new InvalidActionException("Invalid action! You can't select the strongbox!");
        if (parseChoice(source) == -1 || parseChoice(dest) == -1)
            throw new InvalidActionException("Invalid action! Make sure you typed the source and the destination correctly!");
